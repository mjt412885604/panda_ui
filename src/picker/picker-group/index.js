...
export default class PickerGroup extends React.Component {
  ...
  render() {
    ...
    const pickerItem = range.map((item, index) => {
      ...
      return <div className="pandaui-picker__item" key={index}>{`${content}`}</div>
    })

    return (
      <div
        className="pandaui-picker__group"
        onTouchStart={this.onTouchStart}
        onTouchMove={this.onTouchMove}
        onTouchEnd={this.onTouchEnd}
        ref={this.preventDefault}
      >
        <div className="pandaui-picker__mask" />
        <div className="pandaui-picker__indicator" />
        <div className="pandaui-picker__content" style={this.getPosition()}>
          {pickerItem}
        </div>
      </div>
    )
